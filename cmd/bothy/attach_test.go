...
func TestAttachPrefersTheZellijBothyInstalled(t *testing.T) {
	p := sandbox(t, true)
	plan, err := planAttach(p, config.Default(), "/w", "", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAttachHopsOnlyFromOutsideAContainer(t *testing.T) {
	...
	plan, err := planAttach(outside, cfg, "/w", "", []string{"work"})
	if err != nil {
		t.Fatal(err)
	}
	...
	plan, err = planAttach(inside, cfg, "/w", "", []string{"work"})
	if err != nil {
		t.Fatal(err)
	}
	if plan.Container != "" {
...
func TestAttachReportsAMissingMultiplexer(t *testing.T) {
	...
	if _, err := planAttach(p, config.Default(), "/w", "", nil); err == nil {
